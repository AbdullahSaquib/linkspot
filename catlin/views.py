...
from catlin.models import (
    Category, Page, Comment, LikePage, LikeComment, LikeCategory
)
...
def get_search_users(search_string, max_results=25, threshold=0):
    ...
    for user in user_list:
        user_score_list.append(compare_strings(
            search_string, user.user.username))
...
def category(request, category_id):
    ...
    main_comments = Comment.objects.filter(category=category, depth='M').order_by(
        '-last_modified')[comment_index:comment_index+no_of_comments]
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse('catlin:update_category', args=[cat.id]))
    else:
        ...
        if request.POST['type'] == 'N':
            ...
            try:
                parent_comment = Comment.objects.get(
                    id=request.POST['comment_id'], category=category)
...
def like_category(request):
    ...
    json_context = like_my_model(
        my_request=request,
        entity_model=Category,
        like_model=LikeCategory)
...
def see_replies(request):
    ...
    nested_comments = get_list_or_404(
        Comment,
        parent_comment__id=main_comment_id)
    likes = []
    dislikes = []
    usernames = []
    nc_ids = []
